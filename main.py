...
async def check_card_api(card_full):
    ...
    async with api_semaphore:
        try:
            ...
            data = r.json()  # Response كامل من السيرفر
            response = str(data)  # هنعرض كل الردود
...
if not can_user_check(user_id, "single"): await update.message.reply_text("❌ VIP only for single check.") return # Anti-spam فقط للعاديين if user_id not in ADMINS and (user_id not in VIP_USERS or VIP_USERS[user_id] < time.time()): now = time.time() last = last_check_time.get(user_id, 0) if now - last < ANTI_SPAM_SECONDS: await update.message.reply_text(f"❌ Wait {ANTI_SPAM_SECONDS} seconds before next check") return last_check_time[user_id] = now asyncio.create_task(process_pp(update, context)) 
...
if not can_user_check(user_id, "file"): await update.message.reply_text("❌ VIP only for file check.") return asyncio.create_task(process_file(update, context)) 
...
async def process_line(line): nonlocal approved,live,declined match = re.findall(r'\d{12,16}\|\d{2}\|\d{2,4}\|\d{3,4}',line) if not match: return None card_full = match[0] start_time=time.time() status,response = await check_card_api(card_full) await asyncio.sleep(random.uniform(1,5)) taken = round(time.time()-start_time,2) text = await format_response(card_full,status,response,taken) if status=="approved": approved+=1; await update.message.reply_text(text) elif status=="live": live+=1; await update.message.reply_text(text) else: declined+=1 last_info,last_bank,last_country = await get_bin_info(card_full.split("|")[0][:6]) panel = f"""📊 Status 
...
for line in lines: if stop_users.get(user_id): await update.message.reply_text("Stopped ⛔"); return await process_line(line) with open(results_file_path,'w',encoding='utf-8') as result_file: for line in lines: r = await format_response(line.strip(),"N/A","N/A",0) result_file.write(r+"\n\n") await update.message.reply_text(f"Done ✅\nResults saved: {results_file_path}") ------------------- /code ------------------- 
...
VIP_USERS[user_id] = int(time.time()) + code_data["duration"]243600
code_data["used"] += 1
